...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        self.calendarHeaderView.frame = CGRectMake(10,self.calendarWeekdayView.frame.origin.y + weekdayHeight, self.fs_width - 10, headerHeight);
        self.calendarWeekdayView.frame = CGRectMake(0, 0, self.contentView.fs_width, weekdayHeight);
        self.calendarWeekdayView.backgroundColor = [self colorWithHexString:@"F0F0F0"];
        _deliver.frame = CGRectMake(self.calendarHeaderView.fs_left, self.calendarWeekdayView.fs_top, self.calendarHeaderView.fs_width, headerHeight+weekdayHeight);
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (self.floatingMode) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            ...
            stickyHeader.frame = CGRectMake(10, stickyHeader.frame.origin.y, stickyHeader.frame.size.width, stickyHeader.frame.size.height);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDate *minDate = [self getSmallDate];
    if (minDate != nil) {
        [_selectedDates removeAllObjects];
        if (![_selectedDates containsObject:minDate]) {
            [_selectedDates addObject:minDate];
        }
    }
    [_selectedDates addObject:selectedDate];
    NSDate *startingDate = nil; // Earliest date
    NSDate *endingDate = nil; // Latest date
    
    for (id entry in _selectedDates)
    {
        NSDate *date = entry;
    ...
    NSDate *startDate = startingDate;
    NSDate *endDate = endingDate;

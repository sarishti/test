...
//  StayDream
//
//  Created by Sharisti on 02/11/17.
...
class CustomTabBarController: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        tabBar.selectionIndicatorImage = UIImage.imageWithColor(color: #colorLiteral(red: 0.4823529412, green: 0.3254901961, blue: 0.7568627451, alpha: 1), size: tabBarItemSize).resizableImage(withCapInsets: UIEdgeInsets.zero)

        // remove default border
        tabBar.frame.size.width = self.view.frame.width + 4
        tabBar.frame.origin.x = -2

        // set seprate indicator
        if let items = self.tabBar.items {

            //Get the height of the tab bar

            let height = self.tabBar.bounds.height
                //CGRectGetHeight(self.tabBar.bounds)

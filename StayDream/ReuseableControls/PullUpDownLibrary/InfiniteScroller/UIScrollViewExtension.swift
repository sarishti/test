...
public extension UIScrollView {
    ...
    public func addPullRefreshHandler(_ refreshCompletion: (() -> Void)?) {
        self.addPullRefresh(refreshCompletion)
    }
    // Add push/ Infinite scroll handler on table view
    public func addPushRefreshHandler(_ refreshCompletion: (() -> Void)?) {
        self.addPushRefresh(refreshCompletion)
    }

    // Add pull to refresh
    fileprivate func addPullRefresh(_ refreshCompletion: ((() -> Void)?), options: PullToRefreshOption = PullToRefreshOption()) {
    ...
    fileprivate func addPushRefresh(_ refreshCompletion: ((() -> Void)?), options: PullToRefreshOption = PullToRefreshOption()) {

...
extension CameraViewController {
    // To attach the view to the edges of the superview, it needs to be pinned on the sides of the self.view, based on the  edges of this superview. This configure the cameraView to show, in real time, the  camera.
    ...
    //  Add the constraints based on the device orientation, this pin the button on the bottom part of the screen when the device is portrait, when landscape, pin the button on the right part of the screen.
    ...
    // Add the constraints based on the device orientation, centerX the button based on the width of screen.When the device is landscape orientation, centerY  the button based on the height of screen.
    ...
    // Configure the edges constraints of container that handle the center position of SwapButton and LibraryButton.
    ...
    // Configure the gravity of container, based on the orientation of the device.
    ...
    //  Remove the SwapButton constraints to be updated when the device was rotated.
    ...
    //If the device is portrait, pin the SwapButton on the  right side of the CameraButton.If landscape, pin the SwapButton on the top of the CameraButton.
 ...
 // Configure the center of SwapButton, based on the  axis center of CameraButton.
    func configSwapButtonGravityConstraint(_ portrait: Bool) {
        swapButtonGravityConstraint = NSLayoutConstraint(
            ...
            multiplier: 1.0,
            constant: -4.0 * DeviceConfig.SCREENMULTIPLIER)
    ...
    // Add the constraint for the CloseButton, based on the device orientation. If portrait, it pin the CloseButton on the CenterY  of the CameraButton. Else if landscape, pin this button on the Bottom of superview.
    ...
    // Remove the LibraryButton constraints to be updated when the device was rotated.
    ...
    // Add the constraint of the LibraryButton, if the device  orientation is portrait, pin the right side of SwapButton to the left side of LibraryButton.If landscape, pin the bottom side of CameraButton on the top side of LibraryButton.
    ...
    // Set the center gravity of the LibraryButton based on the position of CameraButton.
    ...
    // If the device orientation is portrait, pin the top of FlashButton to the top side of superview. Else if, pin the FlashButton bottom side on the top side of SwapButton.
    ...
    //If the device orientation is portrait, pin the right side of FlashButton to the right side of superview.Else if, centerX the FlashButton on the CenterX of CameraButton.
   ...
   // Used to create a perfect square for CameraOverlay. This method will determinate the size of CameraOverlay,if portrait, it will use the width of superview to determinate the height of the view. Else if landscape,it uses the height of the superview to create the width of the CameraOverlay.
    ...
    // This method will center the relative position of CameraOverlay, based on the biggest size of the superview.
    ...
    // Remove the CameraOverlay constraints to be updated when the device was rotated.
    ...
    // It needs to get a determined smallest size of the screen to create the smallest size to be used on CameraOverlay. It uses the orientation of the screen to determinate where the view will be pinned.
